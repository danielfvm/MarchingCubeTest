...
//
float hash(float n) { return frac(sin(n) * 1e4) *2.-1.; }
float hash(float2 p) { return frac(1e4 * sin(17.0 * p.x + p.y * 0.1) * (0.1 + abs(sin(p.y * 13.0 + p.x)))) *2.-1.; }
...
// float voronoi3D(float3 position, out float3 closestCell) {
//     float3 p = floor(position);
//     float3 f = frac(position);

//     float minDist1 = 8.0; // Closest distance
//     float minDist2 = 8.0; // Second closest distance
//     closestCell = 0;
...
//                 float3 neighborPoint = neighbor + rand3dTo3d(p + neighbor);
//                 float dist = length(f - neighborPoint);
...
//     return minDist2 - minDist1; // Distance to the closest edge
// }

// Generate a normal direction based on Voronoi noise
// float3 voronoiNormal(float3 position) {
//     float3 cellID;
//     float d = voronoi3D(position, cellID);

//     // Compute offsets to sample neighboring points
//     float3 dx = float3(0.01, 0.0, 0.0);
//     float3 dy = float3(0.0, 0.01, 0.0);
//     float3 dz = float3(0.0, 0.0, 0.01);

//     float3 grad;
//     grad.x = voronoi3D(position + dx, cellID) - d;
//     grad.y = voronoi3D(position + dy, cellID) - d;
//     grad.z = voronoi3D(position + dz, cellID) - d;

//     return normalize(grad);
// }

// Optimized verion of the "voronoiNormal", but looks a lot more flat
// float4 voronoiNormalFlat(float3 position) {
// 	float3 p = floor(position);
//     float3 f = frac(position);

//     float minDist1 = 8.0; // Closest distance
//     float minDist2 = 8.0; // Second closest distance
//     float3 closestCell = 0;
//     float3 closestCell2 = 0;

//     for (int z = -1; z <= 1; z++) {
//         for (int y = -1; y <= 1; y++) {
//             for (int x = -1; x <= 1; x++) {
//                 float3 neighbor = float3(x, y, z);
//                 float3 offset = neighbor + rand3dTo3d(p + neighbor);
//                 float dist = length(f - offset);

//                 if (dist < minDist1) {
//                     minDist2 = minDist1;
//                     minDist1 = dist;
//                     closestCell2 = closestCell;
//                     closestCell = p + neighbor;
//                 } else if (dist < minDist2) {
//                     minDist2 = dist;
// 					closestCell2 = p + neighbor;
//                 }
//             }
//         }
//     }

//     // float3 edgeDir = normalize(f - closestOffset); // Direction to closest edge
// 	float3 edgeDir = normalize(closestCell - closestCell2);
//     return float4(edgeDir.xyz, minDist2 - minDist1); // Distance to the closest edge
// }

// float4 voronoiNormalRounded(float3 position, float roundness) {
// 	float3 p = floor(position);
//     float3 f = frac(position);
...
//     // float3 edgeDir = normalize(f - closestCell2); // Direction to closest edge
//     // float3 edgeDir = normalize(closestCell - closestCell2);
// 	float3 edgeDir = normalize(
// 		lerp(
// 			(closestCell - closestCell2), 
// 			(closestCell - position), 
// 			edgeDist * roundness
// 		));
//     return float4(edgeDir.xyz, edgeDist); // Distance to the closest edge
// }
...
// float fbm(float x, int NUM_OCTAVES) {
// 	float v = 0.0;
// 	float a = 0.5;
...
// float fbm(float2 x, int NUM_OCTAVES) {
// 	float v = 0.0;
// 	float a = 0.5;
...
// float fbm(float3 x, int NUM_OCTAVES) {
// 	float v = 0.0;
// 	float a = 0.5;
...
// float fbm(float4 v, int NUM_OCTAVES)
// {
// 	float w=0., d=.5;
...
// A hash function to create pseudo-randomness
// float hash1d(float n) {
//     return frac(sin(n) * 43758.5453);
// }

// 3D hash based on position
// float3 hash3d(float3 p) {
//     return float3(hash1d(dot(p, float3(127.1, 311.7, 74.7))),
//                  hash1d(dot(p, float3(269.5, 183.3, 246.1))),
//                  hash1d(dot(p, float3(113.5, 271.9, 124.6))));
// }
...
//     // Hash the corners of the cube surrounding p
//     float n000 = hash1d(dot(i + float3(0.0, 0.0, 0.0), float3(1.0, 57.0, 113.0)));
//     float n100 = hash1d(dot(i + float3(1.0, 0.0, 0.0), float3(1.0, 57.0, 113.0)));
//     float n010 = hash1d(dot(i + float3(0.0, 1.0, 0.0), float3(1.0, 57.0, 113.0)));
//     float n110 = hash1d(dot(i + float3(1.0, 1.0, 0.0), float3(1.0, 57.0, 113.0)));
//     float n001 = hash1d(dot(i + float3(0.0, 0.0, 1.0), float3(1.0, 57.0, 113.0)));
//     float n101 = hash1d(dot(i + float3(1.0, 0.0, 1.0), float3(1.0, 57.0, 113.0)));
//     float n011 = hash1d(dot(i + float3(0.0, 1.0, 1.0), float3(1.0, 57.0, 113.0)));
//     float n111 = hash1d(dot(i + float3(1.0, 1.0, 1.0), float3(1.0, 57.0, 113.0)));

//     // Interpolate along x, y, and z axes
...
// Generates a smoothly interpolated random float3 based on position
// float3 smoothRandomFloat3(float3 position) {
//     return float3(smoothNoise(position),
//                   smoothNoise(position + float3(31.31, 57.57, 23.23)),
//                   smoothNoise(position + float3(71.71, 13.13, 42.42)));
// }

// float3 smoothRandomFloat3(float3 position) {
//     return float3(snoise(position),
//                   snoise(position + float3(31.31, 57.57, 23.23)),
//                   snoise(position + float3(71.71, 13.13, 42.42)));
// }